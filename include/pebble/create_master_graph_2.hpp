...
void create_master_graph(const Mesh &mesh,
  ...
  for (; saddle != saddles_end; ++saddle) {
    ...
    for (const MeshHalfedge &halfedge : {h, opposite(h, mesh)}) {
      ...
      std::optional<MeshHalfedge> previous_contour;
	...
	if (previous_contour) {
	  assert(previous_contour != up.origin);
	  assert(edge(up.origin, mesh) != edge(*previous_contour, mesh));
	  assert(edge_to_vertex[edge(up.origin, mesh)] != edge_to_vertex[edge(*previous_contour, mesh)]);
	}
	previous_contour = up.origin;
      }
	...
	if (previous_contour) {
	  assert(previous_contour != down.origin);
	  assert(edge(down.origin, mesh) != edge(*previous_contour, mesh));
	  assert(edge_to_vertex[edge(down.origin, mesh)] != edge_to_vertex[edge(*previous_contour, mesh)]);
	}
	previous_contour = down.origin;
	previous_vertex = new_intersection;
      }
